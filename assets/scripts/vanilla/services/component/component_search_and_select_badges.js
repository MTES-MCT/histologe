    ...
    document?.querySelectorAll('.search-and-select-badges-container')?.forEach(container => {
        container.querySelectorAll('.search-and-select-badge-add')?.forEach(element => {
            element.addEventListener('click', (event) => {
                ...
                badge.classList.add('fr-badge', 'fr-badge--blue-ecume', 'fr-m-1v', 'search-and-select-badge-remove')
        ...
        searchInput?.addEventListener('input', (event) => {
            ...
            container.querySelectorAll('.search-and-select-badge-add')?.forEach(element => {
            const normalizedLabel = element.getAttribute('data-badge-label').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
...
const refreshHiddenInput = (container) => {
    ...
    selectedList.forEach(element => {
      if (inputHidden.getAttribute('value') !== '') {
        inputHidden.setAttribute('value', inputHidden.getAttribute('value') + ',' + element.getAttribute('data-badge-id'))
      } else {
