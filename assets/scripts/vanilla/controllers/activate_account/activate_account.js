...
document?.querySelectorAll('.fr-password-toggle')?.forEach((pwdToggle) => {
  pwdToggle.addEventListeners('click touchdown', (event) => {
    ['fr-fi-eye-off-fill', 'fr-fi-eye-fill'].forEach((c) => {
      event.target.classList.toggle(c);
    });
    const pwd = event.target.parentElement.querySelector('[name^="password"]');
    pwd.type !== 'text' ? (pwd.type = 'text') : (pwd.type = 'password');
  });
});
document
  ?.querySelector('form[name="login-creation-mdp-form"]')
  ?.querySelectorAll('[name^="password"]')
  .forEach((pwd) => {
...
document
  ...
  ?.addEventListener('submit', (event) => {
    ...
    if (
      canSubmitFormReinitPassword() &&
      document?.querySelector('form[name="login-creation-mdp-form"] #submitter')
    ) {
      event.target.submit();
    }
  });
...
function canSubmitFormReinitPassword() {
  const pass = document?.querySelector(
    'form[name="login-creation-mdp-form"] #login-password'
  ).value;
  const repeat = document?.querySelector(
    'form[name="login-creation-mdp-form"] #login-password-repeat'
  ).value;
  const pwdMatchError = document?.querySelector(
    'form[name="login-creation-mdp-form"] #password-match-error'
  );
  const submitBtn =
    document?.querySelector('form[name="login-creation-mdp-form"] #submitter') ??
    document?.querySelector('#fr-modal-profil-edit-password #submitter');
  const messageLength = document?.querySelector(
    'form[name="login-creation-mdp-form"] #password-input-message-info-length'
  );
  const messageMaj = document?.querySelector(
    'form[name="login-creation-mdp-form"] #password-input-message-info-maj'
  );
  const messageMin = document?.querySelector(
    'form[name="login-creation-mdp-form"] #password-input-message-info-min'
  );
  const messageNb = document?.querySelector(
    'form[name="login-creation-mdp-form"] #password-input-message-info-nb'
  );
  const messageSpecial = document?.querySelector(
    'form[name="login-creation-mdp-form"] #password-input-message-info-special'
  );
  const groupInputPassword = document?.querySelector(
    'form[name="login-creation-mdp-form"] .fr-input-group-password'
  );
  const groupInputPasswordRepeat = document?.querySelector(
    'form[name="login-creation-mdp-form"] .fr-input-group-password-repeat'
  );
  let canSubmit = true;
