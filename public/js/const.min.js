        ...
        idFetchTimeout = setTimeout( () => {
            if(t.value.length>10)return t.removeEventListener("keyup",searchAddress),
                fetch("https://api-adresse.data.gouv.fr/search/?q="+t.value).then(
                    (t=>{t.json().then((t=>{let r=e.querySelector("#signalement-adresse-suggestion");r.innerHTML="";for(let a of t.features){let t=document.createElement("div");t.classList.add("fr-col-12","fr-p-3v","fr-text-label--blue-france","fr-adresse-suggestion"),t.innerHTML=a.properties.label,t.addEventListener("click",(()=>{e.querySelector("#signalement_adresseOccupant").value=a.properties.name,e.querySelector("#signalement_cpOccupant").value=a.properties.postcode,e.querySelector("#signalement_villeOccupant").value=a.properties.city,e.querySelector("#signalement-insee-occupant").value=a.properties.citycode,e.querySelector("#signalement-geoloc-lat-occupant").value=a.geometry.coordinates[1],e.querySelector("#signalement-geoloc-lng-occupant").value=a.geometry.coordinates[0],r.innerHTML=""
                    ...
                    res.json().then((r) => {
                        e.querySelectorAll('.search-address-autocomplete-list')?.forEach((element) => {
                            ...
                            for (let feature of r.features) {
                                ...
                                suggestion.innerHTML = feature.properties.label;
                                suggestion.addEventListener('click', () => {
                                    e.querySelector('#form-edit-address input[name=adresse-visible]').value = feature.properties.name;
                                    e.querySelector('#form-edit-address input[name=adresse]').value = feature.properties.name;
                                    e.querySelector('#form-edit-address input[name=codePostal-visible]').value = feature.properties.postcode;
                                    e.querySelector('#form-edit-address input[name=codePostal]').value = feature.properties.postcode;
                                    e.querySelector('#form-edit-address input[name=ville-visible]').value = feature.properties.city;
                                    e.querySelector('#form-edit-address input[name=ville]').value = feature.properties.city;
                                    e.querySelector('#form-edit-address input[name=insee]').value = feature.properties.citycode;
                                    e.querySelector('#form-edit-address input[name=geolocLng]').value = feature.geometry.coordinates[0];
                                    e.querySelector('#form-edit-address input[name=geolocLat]').value = feature.geometry.coordinates[1];
                                    element.innerHTML = '';
                                })
