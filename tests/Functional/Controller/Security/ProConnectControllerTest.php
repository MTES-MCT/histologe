...
class ProConnectControllerTest extends WebTestCase
{
    ...
    public function testSuccessfulCallback(): void
    {
        ...
        $session->set('pr_state', 'fake_state');
        $session->set('pr_nonce', 'fake_nonce');
        $session->save();
        ...
        $mockContext->method('clear');
        $container->set(ProConnectContext::class, $mockContext);

        $client->request('GET', '/proconnect/callback?code=fake-code&state=fake_state');
        ...
        /** @var User $user */
        $user = $token->getUser();
        $this->assertSame('1234', $user->getProConnectUid());
