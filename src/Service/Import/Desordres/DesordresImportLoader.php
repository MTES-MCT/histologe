    ...
    public function load(array $data, array $headers, ?OutputInterface $output = null): void
    {
        ...
        foreach ($data as $item) {
            if (\count($item) > 1) {
                ...
                $labelCategorie = $item[DesordresImportHeader::CATEGORIE_LABEL_BO];
                ...
                $desordreCritere = $this->desordreCritereManager->findOneBy(['slugCritere' => $slugCritere]);
                $data = [];
                ...
                $desordreCritere = $this->desordreCritereManager->createOrUpdate($slugCritere, $data);

                $slugPrecision = '' !== $item[DesordresImportHeader::PRECISION_SLUG] ? $item[DesordresImportHeader::PRECISION_SLUG] : $item[DesordresImportHeader::CRITERE_SLUG];

                /** @var DesordrePrecision $desordrePrecision */
                $desordrePrecision = $this->desordrePrecisionManager->findOneBy(['desordrePrecisionSlug' => $slugPrecision]);
                $data = [];
                $data['coef'] = $item[DesordresImportHeader::PRECISION_COEFF];
                $data['danger'] = $item[DesordresImportHeader::PRECISION_DANGER];
                $data['label'] = $item[DesordresImportHeader::PRECISION_CONDITION].' - '.$item[DesordresImportHeader::PRECISION_PIECE];
