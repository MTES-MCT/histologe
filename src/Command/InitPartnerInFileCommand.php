...
#[AsCommand(
    name: 'app:init-partner-in-file',
    description: 'Initialize partner in file entities based on user',
)]
class InitPartnerInFileCommand
{
    ...
    public function __invoke(SymfonyStyle $io): int
    {
        ...
        foreach ($list as $item) {
            ...
            $file = $this->fileRepository->find($item['id']);
        ...
        $this->entityManager->flush();
        $progressBar->finish();
        $io->success($i.' file mis à jour avec un partenaire sur '.$total.'.');
